...
@onready var ip_input     = $MarginContainer/VBoxContainer/CardsContainer/JoinCard/VBoxContainer/IpInput
...
func _on_join_pressed() -> void:
	...
	if not _validate_name(username):
		return
	if ip.is_empty():
		_show_error("Ingresa la IP del host")
		return
	_set_loading(true)
	NetworkManager.join_server(ip, username)
...
func _on_connection_failed() -> void:
	_set_loading(false)
	_show_error("No se pudo conectar. Verifica la IP e intenta de nuevo.")
